...
def _verify_signature(raw_body: bytes, header_sig: Optional[str], header_ts: Optional[str]) -> None:
    ...
    if ENFORCE_TS and header_ts:
        try:
            sent = int(header_ts)
            if abs(int(time.time()) - sent) > TS_SKEW_SEC:
