...
def _is_hidden_run(run) -> List[str]:
    reasons: List[str] = []
    # python-docx exposes font.hidden which maps to w:vanish
    try:
        ...
        pass
    try:
        # Pure white text (best-effort; page bg unknown)
...
def detect_hidden_text(docx_bytes: bytes) -> Dict[str, object]:
    ...
    Samples are texts from hidden runs (capped).
    """
    reasons: List[str] = []
    samples: List[str] = []
    ...
    # TODO: tables/headers/footers if needed later
    return {
