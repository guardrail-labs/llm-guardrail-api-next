...
class FixedWindowQuotaStore:
    """
    Simple fixed-window counters for day and month.
    - Windows reset on UTC day/month boundaries.
    - Counts requests (not tokens) to keep infra light; can be extended later.
    """
    ...
    def _utc_day_start(self, ts: int) -> int:
        # Start of day in UTC (00:00:00)
        return ts - (ts % 86400)

    def _utc_month_start(self, ts: int) -> int:
        # Approximate month start by converting to UTC date then back
    ...
    def check_and_inc(self, key: str) -> QuotaDecision:
        ...
        allowed = day_remaining > 0 and month_remaining > 0
        reason = "ok"
        retry_after = 0
