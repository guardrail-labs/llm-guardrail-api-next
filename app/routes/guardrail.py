...
async def evaluate(request: Request) -> Dict[str, Any]:
    ...
    # Include optional observability fields (safe to ignore by clients)
    body: Dict[str, Any] = {
        "request_id": request_id,
        "action": det["action"],
        "transformed_text": det["transformed_text"],
        "decisions": decisions,
    }
    # Non-breaking extras
    body["risk_score"] = det.get("risk_score", 0)
    body["rule_hits"] = det.get("rule_hits", [])
