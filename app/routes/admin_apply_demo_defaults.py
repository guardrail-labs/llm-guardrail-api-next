...
DEFAULT_DEMO_POLICY = "rules/policies/demo/default.yaml"
...
def _resolved_demo_path() -> str:
    raw = (os.getenv("DEMO_POLICY_PATH") or DEFAULT_DEMO_POLICY).strip()
    if not raw:
        raw = DEFAULT_DEMO_POLICY
    path = Path(raw).expanduser().resolve()
    if not path.is_file():
        raise HTTPException(status_code=404, detail="Demo policy not found.")
    return str(path)
...
@router.post("/bindings/apply_demo_defaults")
def apply_demo_defaults(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    rules_path = _resolved_demo_path()
