...
class IdempotencyMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        # If no key is present, this middleware is a no-op; pass through.
        if not idem_key:
            await self.app(scope, receive, send)
            return

        # If the key is present but invalid, reject.
        ...
        # Fast path: if already stored and body fingerprint matches, replay.
        try:
        ...
        if mismatch:
            ...
            # If someone else re-acquired, execute once without caching to avoid replaying
            # an unrelated payload response.
    ...
    async def _send_raw(
        ...
        *,
        ...
        if idem_key is not None:
            # Always expose whether this was a replay when key is present.
            headers.append(
