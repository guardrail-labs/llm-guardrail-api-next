...
class RedisIdemStore(IdemStore):
    ...
    async def acquire_leader(
        ...
        if ok:
            ...
            if self.recent_limit and self.recent_limit > 0:
                # Keep only the newest N entries (higher scores are newer).
    ...
    async def release(self, key: str) -> None:
        ...
        await self.r.delete(self._k(key, "state"))

    async def meta(self, key: str) -> Mapping[str, Any]:
        state = await self.r.get(self._k(key, "state"))
        lock_value = await self.r.get(self._k(key, "lock"))
        state_text = state.decode() if isinstance(state, bytes) else state
        payload_fp = (
            lock_value.decode() if isinstance(lock_value, bytes) else lock_value
        )
        return {
            "state": state_text,
