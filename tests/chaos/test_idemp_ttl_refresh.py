...
async def test_touch_on_replay_refreshes_ttl(monkeypatch):
    ...
    # First call stores value with TTL ~1s
    await _run(mid, _http_scope(key))
    # Wait a bit, then replay to trigger touch()
    await asyncio.sleep(0.2)
    headers = await _run(mid, _http_scope(key))
    # Store.touch must be called and TTL extended beyond original
