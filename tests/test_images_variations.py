...
def test_images_variations_allow(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        compat,
        "sanitize_text",
    ...
    files = {"image": ("seed.png", fake_png, "image/png")}
    data = {"n": "3", "prompt": "variation"}
    resp = client.post("/v1/images/variations", files=files, data=data)
...
def test_images_variations_deny(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        compat,
        "sanitize_text",
        lambda text, debug=False: (text, [], 0, {}),
