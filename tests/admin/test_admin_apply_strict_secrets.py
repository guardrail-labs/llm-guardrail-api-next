...
def _make_strict(monkeypatch, tmp_path, content: str = 'policy_version: "strict"\n') -> Path:
    ...
    resolved: Path = strict_path.resolve()
    return resolved


def test_apply_strict_first_time(monkeypatch, tmp_path):
    strict_path = _make_strict(monkeypatch, tmp_path)
    ...
    assert data["rules_path"] == str(strict_path)
    assert data["version"]
...
def test_apply_strict_idempotent_refresh(monkeypatch, tmp_path):
    strict_path = _make_strict(monkeypatch, tmp_path, content='policy_version: "1"\n')
    ...
    monkeypatch.setattr(
        "app.routes.admin_apply_strict_secrets.propagate_bindings",
        _record,
    )

    strict_path.write_text('policy_version: "2"\n', encoding="utf-8")
    ...
    assert record["rules_path"] == str(strict_path)
    assert record["policy_version"] == "2"


def test_apply_strict_overwrites_existing(monkeypatch, tmp_path):
...
def test_apply_strict_missing_params(monkeypatch, tmp_path):
    _make_strict(monkeypatch, tmp_path)
...
def test_apply_strict_missing_file(monkeypatch):
    monkeypatch.setenv("STRICT_SECRETS_POLICY_PATH", "/tmp/does-not-exist.yaml")
