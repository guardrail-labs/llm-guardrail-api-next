...
def test_happy_path_evalsha_no_reload() -> None:
    async def _run() -> None:
        client = _make_client()
        ...
        allowed, retry_after, remaining = await bucket.allow(
            "key", cost=1.0, rps=5.0, burst=10.0
        )
...
def test_noscript_reloads_and_retry_succeeds_increments_counter() -> None:
    async def _run() -> None:
        client = _make_client()
...
def test_reload_fails_fall_back_to_eval() -> None:
    async def _run() -> None:
        client = _make_client()
...
def test_reload_succeeds_but_retry_noscript_again_fall_back_to_eval_no_counter_inc() -> None:
    async def _run() -> None:
        client = _make_client()
        client.script_load.side_effect = ["sha1", "sha2"]
...
def test_reload_succeeds_but_retry_non_noscript_propagates_without_eval_or_counter() -> None:
    async def _run() -> None:
        client = _make_client()
        client.script_load.side_effect = ["sha1", "sha2"]
