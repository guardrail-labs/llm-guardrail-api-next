...
inherit the default TTL of 30 minutes (`ADMIN_SESSION_TTL_SECONDS`, defaults to
`1800` and clamped between 5 minutes and 24 hours). When the session cookie is
missing or expires, a new pair is minted and the CSRF token is rotated
alongside it. If the CSRF cookie is cleared while the session remains active, a
new token is issued without rotating the session cookie.
...
flag by setting `ADMIN_COOKIE_SECURE=0`; all other attributes remain
